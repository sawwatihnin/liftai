    ...
    <main className="min-h-screen px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <h1 className="mt-4 max-w-2xl text-4xl font-bold tracking-tight text-white sm:text-5xl">
                Real-time squat coaching with local AI pose analysis.
              </h1>
              <p className="mt-4 max-w-2xl text-base text-slate-300 sm:text-lg">
                Train with live skeleton tracking, squat phase detection, voice coaching, and a
                responsive dashboard. Everything runs in your browser with no backend required.
              </p>
              <div className="mt-6">
                <ControlBar
                  isRunning={isRunning}
            ...
            <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
              <div className="rounded-[28px] border border-white/10 bg-ink/40 p-5">
                ...
                <ul className="mt-3 space-y-2 text-sm text-slate-200">
                  ...
                  <li>Rotate sideways about 30-45 degrees for cleaner hip angles.</li>
                  <li>Grant camera access and start squatting.</li>
                </ul>
              </div>
              <div className="rounded-[28px] border border-white/10 bg-ink/40 p-5">
                <p className="text-sm text-slate-300">Live warnings</p>
                <p className="mt-3 text-xl font-semibold text-white">
                  {snapshot?.feedback.warnings[0] ?? 'All clear'}
                </p>
              ...
              <div className="rounded-[28px] border border-white/10 bg-ink/40 p-5">
                <p className="text-sm text-slate-300">Depth progress</p>
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-white/10">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-aqua via-gold to-coral transition-all"
                    style={{ width: `${snapshot?.squat.depthProgress ?? 0}%` }}
                  />
                </div>
                <p className="mt-2 text-sm text-slate-400">
                  Uses knee flexion to estimate how deep you are in the current rep.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          />
          <Dashboard snapshot={snapshot} sessionDurationMs={sessionDurationMs} />
        </section>
      </div>
    </main>
  );
